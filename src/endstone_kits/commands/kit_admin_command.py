...
class KitAdminCommands:
    ...
    def addcommand(self, sender: CommandSender, args: List[str]) -> None:
        ...
        if len(args) < 2:
            sender.send_message(
                f"{self._config.prefix}Gunakan: /kit addcommand <id> <command...>"
            )
            return
        ...
        try:
            kit = self._kit_manager.add_command(kit_id, template)
        except ValueError as e:
            sender.send_message(f"{self._config.prefix}{e}")
            return

        sender.send_message(
            f"{self._config.prefix}Command '{template}' ditambahkan ke kit "
            f"'{kit.id}' (total {len(kit.commands)} command)."
        )
