...
class KitPlayerCommands:
    ...
    def list_kits(self, sender: CommandSender) -> None:
        ...
        player_uuid = str(sender.unique_id) if isinstance(sender, Player) else None

        lines = [f"{self._config.prefix}Daftar kit ({len(kits)}):"]
        for kit in kits:
            if kit.metadata.one_time:
                already_claimed = player_uuid is not None and (
                    self._cooldown_manager.has_ever_claimed(player_uuid, kit.id)
                )
                status = "sudah diklaim" if already_claimed else "sekali pakai, belum diklaim"
            else:
                status = f"cooldown {kit.metadata.cooldown_seconds}s"
            lines.append(
                f"  - {kit.id} ({kit.metadata.display_name}): "
                f"{len(kit.items)} item, {status}"
            )
        sender.send_message("\n".join(lines))
    ...
    def info(self, sender: CommandSender, kit_id: str) -> None:
        ...
        meta = kit.metadata
        cooldown_text = (
            "tidak berlaku (kit sekali pakai)"
            if meta.one_time
            else f"{meta.cooldown_seconds} detik"
        )
        lines = [
            ...
            f"  Sekali pakai: {'Ya' if meta.one_time else 'Tidak'}",
        ...
        if kit.commands:
            ...
            for i, cmd in enumerate(kit.commands, start=1):
                lines.append(f"    {i}. [{cmd.run_as}] {cmd.template}")
        sender.send_message("\n".join(lines))
    ...
    def claim_kit(self, sender, kit_id: str) -> None:
        ...
        # Kit one-time: sekali diklaim, TIDAK BISA diklaim lagi
        # selama-lamanya, terlepas dari `cooldown_seconds` (yang jadi
        # tidak relevan untuk kit seperti ini). Dicek TERPISAH dari
        # `get_remaining_seconds` di bawah -- kalau `cooldown_seconds`
        # kebetulan 0, pengecekan cooldown biasa akan selalu meloloskan
        # klaim ulang, jadi one-time WAJIB dicek independen di sini.
        if kit.metadata.one_time and self._cooldown_manager.has_ever_claimed(
            player_uuid, kit.id
        ):
            sender.send_message(
                f"{self._config.prefix}{self._config.message('kit_already_claimed')}"
            )
            return
        ...
        # Kit one-time: cabut permission-nya dari player ini (best-
        # effort, lihat peringatan lengkap di
        # `PermissionManager.revoke_permission`). Proteksi klaim ulang
        # SEBENARNYA sudah dijamin oleh pengecekan `has_ever_claimed`
        # di atas -- ini murni efek samping tambahan sesuai permintaan,
        # bukan mekanisme proteksi itu sendiri.
        if kit.metadata.one_time and kit.metadata.permission:
            self._permission_manager.revoke_permission(sender, kit.metadata.permission)
